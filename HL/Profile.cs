...
internal sealed class Profile : Contexted
{
    ...
    {
        if (!resourceLoader.TryGetFileOrWaiter("profile.png", MIME.ImagePng, out ResourceLoader.FileResource _resource, out Waiter<ResourceLoader.FileResource> _resource_waiter))
        {
    ...
    protected override void ErrorCallback(Exception e) => Client.Client.CerrWriteLine(e.ToString());
    ...
    protected override void CleanupCallback() => Client.Client.RenderWriter.DeleteImage(_component_id);
}
