...
public class Contexted
{
    ...
    {
        CancellationToken action_token = CancellationTokenSource.Token;
        _ = Task.Run(() => { action(action_token); }, action_token);
    }
    public void Close() => CloseAsync();
    public Task<Exception> CloseAsync()
    {
        return Task.Run(async () =>
        {
            try
            {
                ...
                _ = _decease_sema.WaitOne();
                _ = _decease_sema.Release();
                //
                ...
                {
                    CleanupCallback();
                    _ = _cleanup_sema.Release();
                }
                //cleanup termination barrier
                _ = _cleanup_sema.WaitOne();
                _ = _cleanup_sema.Release();
            }
            catch (Exception e)
            {
    ...
    protected virtual Task ActivateCallback(CancellationToken token) => Task.CompletedTask;
