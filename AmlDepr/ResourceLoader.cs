...
public class ResourceLoader
{
    public ResourceLoader(AbyssLib.Host host, AbyssURL origin)
    {
        ...
        {
            Tuple<AbyssLib.AbystClient, AbyssLib.DLLError> result = host.GetAbystClient(origin.Id);
    ...
    public bool TryGetFileOrWaiter(string url_string, MIME MimeType, out FileResource resource, out Waiter<FileResource> waiter)
    {
        ...
        if (!AbyssURLParser.TryParseFrom(url_string, Origin, out AbyssURL url))
        {
    ...
    public async Task<HttpResponseMessage> TryHttpGetAsync(AbyssURL url)
    {
        ...
        {
            AbyssLib.AbystResponse raw_response = _abyst_client.Request(AbyssLib.AbystRequestMethod.GET, url.Path);
            ...
            foreach (KeyValuePair<string, string[]> entry in raw_response.Header)
            {
                switch (entry.Key)
                {
