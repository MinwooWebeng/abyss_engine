...
internal class ImageImpl : AmlNode
{
    ...
    protected override Task ActivateSelfCallback(CancellationToken token)
    {
        ...
        if (!_parent_material.MaterialWaiterGroup.TryGetValueOrWaiter(out int material_id, out Waiter<int> material_waiter))
        {
    ...
    [Obsolete]
    protected override void DeceaseSelfCallback() => _resource_waiter?.Finalize(default);
    ...
    private ResourceLoader.FileResource _resource;
